...
public class GwtPluginTest {
  ...
  public void testExtensionAvailable() {
    assertThat(extensions.getByName(GwtBasePlugin.EXTENSION_NAME),
        instanceOf(GwtPluginExtension.class));
  }

  @Test
  public void testConfigurationAvailable() {
    assertNotNull(project.getConfigurations()
        .findByName(GwtBasePlugin.GWT_CONFIGURATION));
  }

  @Test
  public void testBasicTasksAvailable() {
    assertThat(tasks.getByName(GwtCompilerPlugin.TASK_COMPILE_GWT),
        instanceOf(GwtCompile.class));
    assertThat(tasks.getByName(GwtCompilerPlugin.TASK_DRAFT_COMPILE_GWT),
        instanceOf(GwtDraftCompile.class));
  }
  ...
  public void testSuperDevTaskAvailable() {
    ...
    assertThat(tasks.getByName(GwtBasePlugin.TASK_GWT_SUPER_DEV),
        instanceOf(GwtSuperDev.class));
  }
  ...
  public void testWarTasksAvailable() {
    ...
    assertThat(tasks.getByName(GwtWarPlugin.TASK_WAR_TEMPLATE),
        instanceOf(ExplodedWar.class));
    assertThat(tasks.getByName(GwtWarPlugin.TASK_GWT_DEV),
        instanceOf(GwtDev.class));
    assertThat(tasks.getByName(GwtWarPlugin.TASK_DRAFT_WAR),
