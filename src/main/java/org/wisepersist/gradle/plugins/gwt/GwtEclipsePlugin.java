...
public class GwtEclipsePlugin {
  ...
  public void apply(final Project project, final GwtBasePlugin gwtBasePlugin) {
    ...
    final GwtEclipseOptions eclipseExtension = ((ExtensionAware) extension)
        .getExtensions().create("eclipse", GwtEclipseOptions.class);
    ...
    project.getPlugins().withType(GwtWarPlugin.class, warPlugin -> {
      logger.debug(
          "Configuring eclipse model GWT web application settings");

      eclipseModel.getProject()
          .buildCommand(ECLIPSE_BUILDER_WEBAPP_VALIDATOR);

      project.getTasks().getByName(EclipsePlugin.ECLIPSE_TASK_NAME)
          .dependsOn(GwtWarPlugin.TASK_WAR_TEMPLATE);
      project.getTasks().getByName(
          getAssociatedCleanTask(EclipsePlugin.ECLIPSE_TASK_NAME))
          .dependsOn(
              getAssociatedCleanTask(GwtWarPlugin.TASK_WAR_TEMPLATE));

      final GdtOptions gdtExtension = ((ExtensionAware) eclipseExtension)
          .getExtensions().create("gdt", GdtOptionsImpl.class);
      configureGdtExtension(extension, gdtExtension);

      configureGenerateGdt(gdtExtension);

      GenerateGdt generateGdt = project.getTasks()
          .create(GENERATE_GDT_TASK, GenerateGdt.class);
      generateGdt.setSettingsFile(project
          .file(".settings/com.gwtplugins.gdt.eclipse.core.prefs"));
      project.getTasks().getByName(EclipsePlugin.ECLIPSE_TASK_NAME)
          .dependsOn(generateGdt);
    ...
    project.afterEvaluate(p -> {
      eclipseModel.getClasspath().getPlusConfigurations()
          .add(gwtBasePlugin.getGwtConfiguration());
      ...
      if (eclipseExtension.isAddGwtContainer()) {
        ...
        eclipseModel.getClasspath().getMinusConfigurations()
            .add(gwtBasePlugin.getGwtSdkConfiguration());
      } else {
  ...
  private void configureGdtExtension(final GwtPluginExtension extension,
    ...
    ConventionMapping conventionMapping = ((IConventionAware) gdtExtension)
  ...
  private void configureGenerateGdt(final GdtOptions gdtExtension) {
    project.getTasks().withType(GenerateGdt.class, task -> {
      ConventionMapping conventionMapping = ((IConventionAware) task)
          .getConventionMapping();
      conventionMapping.map("warSrcDir",
          (Callable<File>) () -> gdtExtension.getWarSrcDir());
      conventionMapping.map("warSrcDirIsOutput",
          (Callable<Boolean>) () -> gdtExtension.getWarSrcDirIsOutput());
      conventionMapping.map("lastWarOutDir",
          (Callable<File>) () -> gdtExtension.getLastWarOutDir());
    });
  }
